...
type ChannelPointsCustomRewardRemoveEvent struct {
	...
	MaxPerStream                      interface{} `json:"max_per_stream"`                        // Whether a maximum per stream is enabled and what the maximum is.
	MaxPerUserPerStream               interface{} `json:"max_per_user_per_stream"`               // Whether a maximum per user per stream is enabled and what the maximum is.
	BackgroundColor                   string      `json:"background_color"`                      // Custom background color for the reward.
	Image                             interface{} `json:"image"`                                 // Set of custom images of 1x, 2x and 4x sizes for the reward.
	DefaultImage                      interface{} `json:"default_image"`                         // Set of default images of 1x, 2x and 4x sizes for the reward.
	GlobalCooldown                    interface{} `json:"global_cooldown"`                       // Whether a cooldown is enabled and what the cooldown is in seconds.
...
type ChannelPointsCustomRewardRemoveEventCondition struct{}
